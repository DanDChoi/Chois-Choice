...
public class FileUtilz {
    public static String fileName = "";
   	...
   	public static void copyFile(File from, File to) throws IOException {
   		...
   		try {
   			...
   			throw new IOException(
   					"Utilities.copyFile: opening input stream '" + from.getPath() + "', " + ex.getMessage(), ex);
   		}
   		...
   		try {
   			...
   			throw new IOException(
   					"Utilities.copyFile: opening output stream '" + to.getPath() + "', " + ex.getMessage(), ex);
   		}
   	...
   	public static void copyInputToOutput(InputStream input, OutputStream output, long byteCount) throws IOException {
   		...
   		for (length = byteCount; length > 0;) {
   			...
   			try {
   				...
   				throw new IOException("Writing output stream, " + ex.getMessage(), ex);
   			}
   		}
   	...
   	public static long getFileSize(String path, String name) {
   		...
   		try {
   			if (existFile(path, name)) {
   				...
   				// log.debug("파일이 미 존재!" + path + name, null);
   				i = 0;
   			}
   		} catch (Exception e) {
   			i = 0;
   			...
   			if (existDirectory(oldPath)) {
   				...
   				if (f2 != null && f2.length > 0) {
   					for (int j = 0; j < f2.length; j++) {
   						...
   						// log.debug("현재 " + j + "/" + f2.length + " 진행중 입니다",
   						// null);
   					}
   				}
   			}
   	...
   	public static String getFileSizeStr(String filesize) {
   		return getFileSize2(StringUtilz.strToDouble(filesize));
   	}
   	...
   	public void copyUniqueElements(String sourceFile, String resultFile) {
   		...
   		v = MiscUtilz.removeDuplicates(v);
